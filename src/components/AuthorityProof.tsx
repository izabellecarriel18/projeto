      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="md:hidden overflow-x-auto scrollbar-hide px-4">
          <div className="flex gap-4 pb-4" style={{ width: 'max-content' }}>
            {metrics.map((metric, index) => (
              <div
                key={index}
                className="bg-black border-2 border-red-600 rounded-2xl p-8 text-center hover:scale-105 transition-all flex-shrink-0"
                style={{ width: '280px' }}
              >
        ...
        <div className="hidden md:grid md:grid-cols-3 gap-6 px-4">
          {metrics.map((metric, index) => (
            <div
              key={index}
              className="bg-black border-2 border-red-600 rounded-2xl p-12 text-center hover:scale-105 transition-all"
            >
